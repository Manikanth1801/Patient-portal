...
class ExistingUsers extends Component {
  ...
  activateDeactivate(e, status, userId) {
    ...
    // let token = localStorage.getItem("accessToken");
    // axios
    //   .patch("http://localhost:8000/users/1", postData, {
    //     headers: {
              ...
              userData.map((user, index) => (
                <tr key={index}>
                  ...
                  <td>
                    {user.isRegistered ? (
                      <div
                        className="common-btn deactive-btn"
                        onClick={(e) =>
                          this.activateDeactivate(e, "deactivate", user.id)
                        }
                      >
                        Deactivate
                      </div>
                      ...
                      <div
                        className="common-btn active-btn"
                        onClick={(e) =>
                          this.activateDeactivate(e, "activate", user.id)
                        }
                      >
                        Activate
                      </div>
